...
module.exports.createAdmin = async (req, res) => {
  ...
  if (user && user.activated) {
    ...
    try {
      ...
      const expTimeToken = jwt.sign(payload, process.env.SECRET, {
        expiresIn: "1hr",
      });
...
module.exports.updateClient = async (req, res) => {
  ...
  try {
    // user = await User.
    user1 = await Client.findByIdAndUpdate(
      id,
      { disabled: disabled1, skills: "asd" },
      { new: true }
    );
    console.log(user1);
...
module.exports.getStaff = async (req, res) => {
  ...
  try {
    ...
    console.log({
      staff: userWithStaff.staff.map((staff) =>
...
module.exports.getClient = async (req, res) => {
  ...
  try {
    ...
    console.log({
      client: userWithClient.client.map((client) =>
...
module.exports.signin = async (req, res) => {
  ...
  if (!admin.activated)
    return res.status(401).json("Kindly verify your account first!");
  console.log(admin);
  const validPassword = await bcrypt.compare(password, admin.password);
  if (!validPassword)
    return res.status(401).json(`The password you entered is incorrect!`);
...
module.exports.adminAuth = async (req, res, next) => {
  ...
  try {
    ...
    if (!admin) {
      return res.json({ err: `Your token has expired!` });
    }
      ...
      if (date.getTime() < decodedToken.expTime) {
        ...
        try {
          ...
          const updatedUser = await User.findOneAndUpdate(
            { temporaryToken: expTimeTokenBody },
            {
              activated: true,
            },
            { new: true }
        ...
        var mailOptions = {
          ...
          html:
            `<body style="border-radius:2%;border: 1px solid orangered;background:linear-gradient(90deg, rgba(2,0,36,1) 0%, rgba(81,92,114,1) 53%, rgba(67,73,73,1) 100%);padding: 30px;"><h1 style="color: rgb(177, 59, 59)">Hello ` +
            admin.rows[0].name +
            `,</h1><h3 style="color: rgb(146, 146, 158)">Your account has been activated successfully!</h3></body>`,
        };
